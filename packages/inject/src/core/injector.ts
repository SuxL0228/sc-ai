...
import {
  ...
  simulateRealClick,
  wait,
  isTimerWorkerDegraded,
} from './dom-utils.js';

// #region debug-point send-flow（临时探针：定位各平台发送流程卡点，验证后移除）
function dbgSendFlow(step: string, data: Record<string, unknown>) {
  try {
    void fetch('http://127.0.0.1:7777/event', {
      ...
      body: JSON.stringify({
        sessionId: 'send-flow',
        location: 'injector.ts',
        msg: step,
        data: {
          ...data,
          visibility: document.visibilityState,
          timerWorkerDegraded: isTimerWorkerDegraded(),
...
function emitSSEEnd() {
  ...
  if (sseState.chunkCount > 0 && currentCallbacks?.onComplete) {
    ...
    // 会话 ID 此前缺失的话再读一次 URL（标签页中途可见过的话此时已更新）
    const finalId = sseConversationId
      ?? (currentProvider ? getConversationId(currentProvider) : undefined);
    currentCallbacks.onComplete(fullText, finalId);
  }
}
...
function setupSSEInterceptor() {
  ...
  if (!alreadyHooked) {
    ...
    XMLHttpRequest.prototype.send = function () {
      ...
      // 用 Worker 计时循环代替 setInterval：后台标签页中 setInterval 会被钳制到
      // 1s/60s 一次，导致流式增量长时间不被消费
...
/**
 * 等待"发送已被站点接受"的信号：会话 ID 出现（URL 变化）或首个流式响应到达。
 *
 * 部分站点（元宝/文心）的前端路由把 URL 更新挂在 requestAnimationFrame 上，
 * 而 rAF 在隐藏标签页中完全暂停——消息实际发送成功、回答正常流式返回，
 * 但 URL 始终不变。因此 SSE 首包同样视为发送成功的铁证。
 */
async function waitForSendAccepted(
  provider: ProviderConfig,
  timeout = 5000
): Promise<{ conversationId?: string; sseStarted: boolean }> {
  const start = Date.now();

  let id = getConversationId(provider);
  while (!id && Date.now() - start < timeout) {
    if (sseState.chunkCount > 0) {
      return { conversationId: undefined, sseStarted: true };
    }
    await wait(100);
    id = getConversationId(provider);
  }

  return { conversationId: id, sseStarted: sseState.chunkCount > 0 };
}
/**
 * 监听 AI 回复（仅 SSE 拦截）
 *
  ...
  return {
    ...
    async send(
      ...
      if (isFullSend && message) {
        dbgSendFlow('send-entry', { providerId: provider.id, options });

        // 追问模式：不点新对话按钮，直接在输入框填入新消息
        if (!options?.continueChat) {
          // 1. 如果需要新对话，先开启新对话
          if (options?.newChat) {
            const newChatResult = await this.newChat();
            dbgSendFlow('new-chat', { providerId: provider.id, result: newChatResult });
          ...
          if (options?.thinking !== undefined && provider.actions.thinking) {
            // #region debug-point A:thinking-sync-entry
            void fetch('http://127.0.0.1:7777/event', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'deepseek-thinking-toggle', runId: 'pre-fix', hypothesisId: 'A', location: 'injector.ts:623', msg: '[DEBUG] enter thinking sync', data: { providerId: provider.id, expectedThinking: options.thinking }, ts: Date.now() }) }).catch(() => {});
            // #endregion
            const thinking = createThinkingCapability(provider);
            if (thinking) {
              const thinkingResult = await (options.thinking ? thinking.enable() : thinking.disable());
              // #region debug-point A:thinking-sync-result
              void fetch('http://127.0.0.1:7777/event', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'deepseek-thinking-toggle', runId: 'pre-fix', hypothesisId: 'A', location: 'injector.ts:626', msg: '[DEBUG] thinking sync finished', data: { providerId: provider.id, expectedThinking: options.thinking, result: thinkingResult }, ts: Date.now() }) }).catch(() => {});
              // #endregion
              await wait(300);
            }
          }
          ...
          dbgSendFlow('fill', { providerId: provider.id, result: fillResult });
          ...
          // 追问模式：直接填充消息，不操作新对话/思考/搜索开关
          const fillResult = await this.fill(message);
          dbgSendFlow('fill-continue', { providerId: provider.id, result: fillResult });
          if (!fillResult.success) {
            return {
              success: false,
              reason: `fill-failed: ${fillResult.reason}`,
    ...
    async _send(callbacks?: SendCallbacks) {
      ...
      dbgSendFlow('locate', { providerId: provider.id, inputFound: !!inputEl, sendBtnFound: !!sendBtn });
      ...
      if (callbacks) {
        ...
        setupSSEInterceptor();
        // 清掉上一轮残留状态：chunkCount 要作为"本轮流式输出已开始"的发送成功信号
        resetSSEState();
      ...
      console.log(`[AI Clash Inject] ${provider.id}: 开始等待发送确认（会话 ID 或首个流式响应）...`);
      // 隐藏标签页里站点自身的路由代码会被限速，URL 更新可能明显滞后，放宽预算
      const accepted = await waitForSendAccepted(provider, document.hidden ? 20000 : 8000);
      const conversationId = accepted.conversationId;
      dbgSendFlow('conversation-id', { providerId: provider.id, conversationId: conversationId || null, sseStarted: accepted.sseStarted });

      if (!conversationId && !accepted.sseStarted) {
        console.warn(`[AI Clash Inject] ${provider.id}: 等待发送确认超时，当前 URL:`, window.location.href);
        // 既没有会话 ID 也没有流式响应，视为失败
      ...
      if (conversationId) {
        ...
        console.log(`[AI Clash Inject] ${provider.id}: 未取到会话 ID，但流式响应已开始，按发送成功处理`);
      }

      // 触发回调（conversationId 可能为 undefined，此时仅作为"发送成功"信号）
...
function createThinkingCapability(provider: ProviderConfig): Capabilities['thinking'] {
  ...
  return {
    ...
    async enable() {
      ...
      // #region debug-point A:thinking-enable-current
      void fetch('http://127.0.0.1:7777/event', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'deepseek-thinking-toggle', runId: 'pre-fix', hypothesisId: 'A', location: 'injector.ts:838', msg: '[DEBUG] thinking enable current state', data: { providerId: provider.id, current }, ts: Date.now() }) }).catch(() => {});
      // #endregion
      if (!current.found) {
      ...
      // #region debug-point E:thinking-enable-result
      void fetch('http://127.0.0.1:7777/event', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'deepseek-thinking-toggle', runId: 'pre-fix', hypothesisId: 'E', location: 'injector.ts:845', msg: '[DEBUG] thinking enable impl result', data: { providerId: provider.id, success }, ts: Date.now() }) }).catch(() => {});
      // #endregion
