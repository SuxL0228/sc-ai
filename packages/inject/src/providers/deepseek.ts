...
import { findAnyElement, hasClass, simulateRealClick, wait } from '../core/dom-utils.js';

function findToggleButton(label: string): HTMLElement | null {
  // DeepSeek 的开关是 <div tabindex="0" aria-pressed="..."> 结构，没有 role="button"
  const selectors = [
  ...
  // #region debug-point B:find-toggle-button
  void fetch('http://127.0.0.1:7777/event', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'deepseek-thinking-toggle', runId: 'pre-fix', hypothesisId: 'B', location: 'deepseek.ts:15', msg: '[DEBUG] deepseek find toggle button', data: { label, foundCandidate: Boolean(el), candidateTag: (el as HTMLElement | null)?.tagName || null, candidateClass: (el as HTMLElement | null)?.className || null }, ts: Date.now() }) }).catch(() => {});
  // #endregion
  if (!el) return null;
  return (el.closest('.ds-toggle-button, [role="button"], button') as HTMLElement | null) || (el as HTMLElement);
}

function isToggleEnabled(button: HTMLElement | null): boolean {
  if (!button) return false;
  return hasClass(button, 'ds-toggle-button--selected')
    || button.getAttribute('aria-pressed') === 'true'
    || button.getAttribute('data-state') === 'checked';
}

async function syncToggle(label: string, expectedEnabled: boolean): Promise<boolean> {
  const button = findToggleButton(label);
  // #region debug-point C:sync-toggle-entry
  void fetch('http://127.0.0.1:7777/event', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'deepseek-thinking-toggle', runId: 'pre-fix', hypothesisId: 'C', location: 'deepseek.ts:29', msg: '[DEBUG] deepseek sync toggle entry', data: { label, expectedEnabled, foundButton: Boolean(button), buttonClass: button?.className || null, enabledBefore: isToggleEnabled(button) }, ts: Date.now() }) }).catch(() => {});
  // #endregion
  if (!button) return false;
  if (isToggleEnabled(button) === expectedEnabled) return true;

  simulateRealClick(button);
  await wait(250);

  let latestButton = findToggleButton(label);
  // #region debug-point C:sync-toggle-after-first-click
  void fetch('http://127.0.0.1:7777/event', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'deepseek-thinking-toggle', runId: 'pre-fix', hypothesisId: 'C', location: 'deepseek.ts:37', msg: '[DEBUG] deepseek sync toggle after first click', data: { label, expectedEnabled, foundButton: Boolean(latestButton), enabledAfterFirstClick: isToggleEnabled(latestButton) }, ts: Date.now() }) }).catch(() => {});
  // #endregion
  if (isToggleEnabled(latestButton) === expectedEnabled) return true;

  // DeepSeek 页面初次打开时按钮状态偶尔会延迟刷新，补一次重试。
  await wait(350);
  latestButton = findToggleButton(label);
  if (!latestButton) return false;
  if (isToggleEnabled(latestButton) === expectedEnabled) return true;

  simulateRealClick(latestButton);
  await wait(250);
  latestButton = findToggleButton(label);
  // #region debug-point C:sync-toggle-final
  void fetch('http://127.0.0.1:7777/event', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'deepseek-thinking-toggle', runId: 'pre-fix', hypothesisId: 'C', location: 'deepseek.ts:50', msg: '[DEBUG] deepseek sync toggle final state', data: { label, expectedEnabled, foundButton: Boolean(latestButton), enabledFinal: isToggleEnabled(latestButton) }, ts: Date.now() }) }).catch(() => {});
  // #endregion
  return isToggleEnabled(latestButton) === expectedEnabled;
}

// 思考模式实现
const thinkingAction: ToggleAction = {
  async getState() {
    const button = findToggleButton('深度思考');
    // #region debug-point B:thinking-get-state
    void fetch('http://127.0.0.1:7777/event', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'deepseek-thinking-toggle', runId: 'pre-fix', hypothesisId: 'B', location: 'deepseek.ts:61', msg: '[DEBUG] deepseek thinking getState', data: { foundButton: Boolean(button), enabled: isToggleEnabled(button), buttonClass: button?.className || null, ariaPressed: button?.getAttribute('aria-pressed') || null }, ts: Date.now() }) }).catch(() => {});
    // #endregion
    if (!button) return { found: false, enabled: false };
    return { found: true, enabled: isToggleEnabled(button) };
  },

  async enable() {
    return syncToggle('深度思考', true);
  },

  async disable() {
    return syncToggle('深度思考', false);
  },
};
...
const searchAction: ToggleAction = {
  async getState() {
    const button = findToggleButton('智能搜索');
    if (!button) return { found: false, enabled: false };
    return { found: true, enabled: isToggleEnabled(button) };
  },

  async enable() {
    return syncToggle('智能搜索', true);
  },

  async disable() {
    return syncToggle('智能搜索', false);
  },
};
