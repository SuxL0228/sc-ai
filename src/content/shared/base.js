/**
 * Content Script 基座 - 基于 @sc-ai/inject 实现
 *
...
export function bootstrapProvider(providerId) {
  ...
  let activeOutputProvider = providerId;
    ...
    window.addEventListener('message', (event) => {
      ...
      if (event.data.type === '__aiclash_sse_chunk') {
        ...
        safeSend({
          ...
          payload: { provider: activeOutputProvider, text, stage, isThink }
        });
        return;
      }
      ...
      if (event.data.type === '__aiclash_complete') {
        ...
        safeSend({
          ...
          payload: { provider: activeOutputProvider }
        });
        return;
      }
      ...
      if (event.data.type === '__aiclash_error') {
        ...
        safeSend({
          ...
          payload: { provider: activeOutputProvider, message: error, errorType: errorType || 'system_error' }
        });
        return;
      }
    });
    ...
    const rpcCapabilities = {
      chat: async (method, ...args) => {
        ...
        if (method === 'send' && args.length >= 3) {
          ...
          return new Promise((resolve, reject) => {
            // 外层兜底超时需大于 MAIN 世界内部各步骤预算之和（后台页等会话 ID 已放宽到 20s）
            const timeoutId = setTimeout(() => {
              logger.error(`[AI Clash ${PROVIDER}] ✗ 等待会话 ID 超时 (40s)，可能是页面响应慢或 SSE 拦截失败`);
              pendingCallbacks.delete(seq);
              pendingResolves.delete(seq);
  ...
  async function executeTask(prompt, settings) {
    activeOutputProvider = settings?.outputProviderId || PROVIDER;
    // 初始化注入器
    const caps = await initInjector();
    if (!caps) {
      safeSend({
        type: MSG_TYPES.ERROR,
        payload: { provider: activeOutputProvider, message: '注入器初始化失败' }
      });
    ...
    safeSend({
      type: MSG_TYPES.CHUNK_RECEIVED,
      payload: { provider: activeOutputProvider, text: '', stage: 'connecting' }
    });
    ...
    try {
      ...
      safeSend({
        type: MSG_TYPES.CHUNK_RECEIVED,
        payload: { provider: activeOutputProvider, text: '正在发送消息...', stage: 'connecting', isStatus: true }
      });

      const options = {
        newChat: settings?.isNewConversation !== false,
        continueChat: !!settings?.continueChat,
      ...
      const sendResult = await caps.chat('send', prompt, options, {
        ...
        onConversationId: (conversationId) => {
          ...
          safeSend({
            type: MSG_TYPES.CHUNK_RECEIVED,
            payload: { provider: activeOutputProvider, text: '发送完成，等待 AI 回复...', stage: 'waiting', isStatus: true }
          });
        },
      ...
      safeSend({
        type: MSG_TYPES.CHUNK_RECEIVED,
        payload: { provider: activeOutputProvider, text: '', stage: 'connecting', isStatus: true, clearError: true }
      });

    } catch (err) {
      safeSend({
        type: MSG_TYPES.ERROR,
        payload: { provider: activeOutputProvider, message: err.message || '任务执行失败' }
      });
      safeSend({
        type: MSG_TYPES.TASK_COMPLETED,
        payload: { provider: activeOutputProvider }
