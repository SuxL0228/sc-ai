...
import { SUMMARY_SYSTEM_PROMPT, buildSummaryUserContent, buildWebSummaryPrompt } from '../shared/summaryPrompt.js';
...
// 派发 Web 任务：导播模式需要串行（依次抢占激活/聚焦），纯后台模式各通道
// 在各自 tab 中互不冲突，并行派发让总耗时从"相加"变"取最大"
function dispatchWebTask(task, label, exclusive) {
  if (exclusive) {
    return enqueueWebTask(task, label);
  }
  return (async () => {
    beginRequest();
    try {
...
async function handleSummaryRequest(question, responses, summaryConfig) {
  const { providerId, customPrompt, activateTab = false, isDeepThinkingEnabled = true } = summaryConfig;
  const provider = getProvider(providerId);
  if (!provider || provider.hasContentScript === false) {
    throw new Error('归纳总结通道无效，请在设置中选择可用的网页 AI');
  }
  ...
  try {
    const loginState = await getProviderLoginState(providerId);
    if (loginState?.status !== 'logged_in') {
      throw new Error(loginState?.message || `请先登录 ${provider.name} 后再进行归纳总结`);
    }

    const webSummaryPrompt = buildWebSummaryPrompt(question, validResponses, customPrompt ?? SUMMARY_SYSTEM_PROMPT);
    const userContent = buildSummaryUserContent(question, validResponses);
    logger.log(`[AI Clash] 使用 ${provider.name} 网页通道执行归纳总结`, {
      providerId,
      activateTab: !!activateTab,
      deepThinking: !!isDeepThinkingEnabled,
      promptLength: webSummaryPrompt.length,
      sourceLength: userContent.length,
    });

    await dispatchWebTask(async () => {
      await routeToTab(provider, webSummaryPrompt, {
        isNewConversation: true,
        isDeepThinkingEnabled: !!isDeepThinkingEnabled,
        isWebSearchEnabled: false,
        outputProviderId: '_summary',
      }, !!activateTab, '_summary');
    }, `summary:${providerId}`, !!activateTab);
  } catch (error) {
    chrome.runtime.sendMessage({
      type: MSG_TYPES.ERROR,
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.type === MSG_TYPES.DISPATCH_TASK) {
        const { provider: providerId, prompt, settings, mode, activateTab = false } = request.payload;
        const provider = getProvider(providerId);
        if (!provider) {
    ...
    if (request.type === MSG_TYPES.TRY_FOCUS_FOLLOW) {
        if (!request.payload?.enabled) return false;
...
// content script 在线但任务执行失败（区别于"脚本未注入/已失效"，后者才需要重载页面）
class ContentScriptExecError extends Error {}

// 核心路由逻辑：寻找或新建标签页
async function injectContentScriptAndSendMessage(tabId, provider, msg, outputProviderId = provider.id) {
    try {
        // 先尝试直接发送消息，并等待 content script 响应
        const response = await chrome.tabs.sendMessage(tabId, msg);
        if (!response?.ok) {
            // content script 活着但执行失败：这是业务错误（如等待会话 ID 超时），
            // 不能当作"脚本已死"去重载页面——那会打断正在输出的回答并造成重复发送
            throw new ContentScriptExecError(response?.error || '任务执行失败');
        }
        // 消息发送成功，更新为 sending 阶段
        chrome.runtime.sendMessage({
            type: MSG_TYPES.TASK_STATUS_UPDATE,
            payload: { provider: outputProviderId, stage: 'sending', text: `消息已发送，等待${provider.name}响应...` }
        }).catch(() => {});
        logger.log(`[AI Clash] ${provider.id} 消息发送成功，等待执行...`);
    } catch (err) {
        if (err instanceof ContentScriptExecError) {
            // 执行失败但脚本在线：直接上报真实原因，不重载页面
            logger.error(`[AI Clash] ${provider.id} 任务执行失败:`, err.message);
            sendProviderError(outputProviderId, `${provider.name} 执行失败：${err.message}`);
            return;
        }
        ...
        chrome.runtime.sendMessage({
            type: MSG_TYPES.TASK_STATUS_UPDATE,
            payload: { provider: outputProviderId, stage: 'loading', text: `正在重新加载${provider.name}页面...` }
        }).catch(() => {});
        ...
        try {
            ...
            chrome.runtime.sendMessage({
                type: MSG_TYPES.TASK_STATUS_UPDATE,
                payload: { provider: outputProviderId, stage: 'sending', text: `消息已发送（重载后），等待${provider.name}响应...` }
            }).catch(() => {});
            ...
            sendProviderError(outputProviderId, `${provider.name} 页面重载后仍无法连接：${err.message}`);
        }
    }
}
...
async function openAndActivateTab(provider, activate = true) {
    ...
    if (rememberedTabId && await isTabValid(rememberedTabId, provider)) {
        ...
        await preventTabDiscard(rememberedTabId);
        if (activate) {
            // 导播模式开启时才主动切换至目标页
    ...
    if (newTab.id) {
        ...
        await saveProviderTabMap();
        await preventTabDiscard(newTab.id);
    }
    return { success: true, tabId: newTab.id };
}

// 禁止浏览器（内存节省模式等）丢弃 AI 通道标签页——被丢弃后 content script 会整个消失
async function preventTabDiscard(tabId) {
    try {
        await chrome.tabs.update(tabId, { autoDiscardable: false });
    } catch (e) {
        logger.warn(`[AI Clash] 设置 autoDiscardable 失败 (tab ${tabId}):`, e);
    }
}
...
async function routeToTab(provider, prompt, settings, activateTab = false, outputProviderId = provider.id) {
    ...
    // 1. 先打开或复用 tab；仅在导播模式开启时激活并聚焦页面
    logger.log(`[AI Clash] ${provider.id} 正在打开/复用 tab，activateTab=${activateTab}`);
    chrome.runtime.sendMessage({
        type: MSG_TYPES.TASK_STATUS_UPDATE,
        payload: {
            provider: outputProviderId,
    ...
    const tabResult = await openAndActivateTab(provider, activateTab);
    logger.log(`[AI Clash] ${provider.id} openAndActivateTab 完成：`, tabResult);
    if (!tabResult.success || !tabResult.tabId) {
        sendProviderError(outputProviderId, `无法打开${provider.name}页面`);
        return;
    }
    ...
    await injectContentScriptAndSendMessage(tabId, provider, msg, outputProviderId);
