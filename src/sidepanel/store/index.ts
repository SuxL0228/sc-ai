...
export const useStore = create<AppStore>()((set, get) => {
  ...
  const saveSummaryConfig = (config?: SummaryConfig) => {
    const s = get();
    chrome.storage?.local.set({
      [SUMMARY_CONFIG_KEY]: config || { providerId: s.summaryProviderId, model: s.summaryModel },
    });
  };
  ...
  const getAvailableSummaryProviderIds = () => PROVIDER_META
    .filter((p: any) => p.id !== 'summarizer' && (PROVIDER_IDS as readonly string[]).includes(p.id))
    .map((p: any) => p.id as ProviderId);

  const isSummaryTargetSelectable = (providerId?: string) => {
  ...
  const resolveSummaryConfig = (
    ...
    if (providerId && isSummaryTargetSelectable(providerId)) {
      return { providerId, model: '' };
    }

    const availableProviderIds = getAvailableSummaryProviderIds();
    const fallbackProviderId = availableProviderIds[0];
  ...
  const syncSummaryConfig = () => {
    const s = get();
    const nextConfig = resolveSummaryConfig(s.summaryProviderId, s.summaryModel);
    if (nextConfig.providerId === s.summaryProviderId && nextConfig.model === s.summaryModel) return;
    set({
      summaryProviderId: nextConfig.providerId || '',
      summaryModel: nextConfig.model || '',
    });
    saveSummaryConfig(nextConfig);
  };
    ...
    await new Promise<void>((resolve, reject) => {
      const msg = {
        ...
        payload: {
          provider: providerId,
          prompt,
          activateTab: s.isFocusFollowEnabled,
          ...
          settings: {
            ...
            conversationHistory,
            isNewConversation,
            continueChat: !isNewConversation,
          },
        },
      };
  ...
  return {
    ...
    setSummaryProviderId: (v) => {
      const nextProviderId = v || '';
      const nextConfig = resolveSummaryConfig(nextProviderId, '');
      set({
        summaryProviderId: nextConfig.providerId || '',
        summaryModel: nextConfig.model || '',
      });
    ...
    setSummaryModel: (v) => {
      const nextConfig = resolveSummaryConfig(get().summaryProviderId, v);
      set({ summaryModel: nextConfig.model || '' });
      saveSummaryConfig(nextConfig);
    },
    ...
    retryProvider: async (providerId) => {
      ...
      const isMultiTurn = s.isMultiTurnSession && s.hasAsked && !s.isCurrentSessionFromHistory;
      // 重试时只传该 provider 自己的对话历史
      const conversationHistory = isMultiTurn
        ? s.conversationTurns.filter(t => t.providerId === providerId && t.response).map(t => ({ question: t.question, response: t.response }))
        : [];
      const isNewConversation = !isMultiTurn;
    ...
    submit: async () => {
      ...
      // Build per-provider conversation turns and a shared history for dispatch
      let newTurns = [...s.conversationTurns];
      let newSessionId = s.activeSessionId;
      let newIsMultiTurn = s.isMultiTurnSession;

      if (isMultiTurn) {
        // Append completed turns for enabled providers that had responses
        for (const id of PROVIDER_IDS) {
          if (!s.enabledMap[id]) continue;
          const lastTurn = s.conversationTurns.find(t => t.providerId === id);
          if (lastTurn && lastTurn.response) {
            newTurns = newTurns.filter(t => t.providerId !== id);
            newTurns.push({
              question: s.currentQuestion, providerId: id,
              response: buffers.fullText[id] || s.responses[id] || lastTurn.response,
              thinkResponse: buffers.thinkText[id] || '',
              rawUrl: s.rawUrlMap[id] || lastTurn.rawUrl || '',
              stats: s.statsMap[id] ?? null,
            });
          }
        }
      ...
      // Build per-provider conversation history for each provider task
      const providerHistoryMap: Record<string, Array<{ question: string; response: string }>> = {};
      for (const turn of newTurns) {
        if (!turn.response || turn.response === s.currentQuestion) continue;
        if (!providerHistoryMap[turn.providerId]) {
          providerHistoryMap[turn.providerId] = [];
        }
        providerHistoryMap[turn.providerId].push({ question: turn.question, response: turn.response });
      }
      const isNewConversation = !isMultiTurn;
      ...
      const taskPromises = enabledIds.map(async (id) => {
        ...
        return dispatchProviderTask(id, prompt, s, providerHistoryMap[id] || [], isNewConversation);
      });
    ...
    isSummaryConfigValid: () => {
      ...
      return isSummaryTargetSelectable(pid);
    },
    summaryBlockReason: () => {
      const s = get();
      if (!s.isSummaryConfigValid()) return '请先选择一个用于网页归纳总结的 AI';
      if (PROVIDER_IDS.filter(id => s.enabledMap[id]).length < 2) return '请选择 2 个或以上通道';
      return null;
    },
    getSummaryProviderOptions: () => {
      return getAvailableSummaryProviderIds()
        .map((providerId) => ({ value: providerId, label: PROVIDER_NAME_MAP[providerId] || providerId }));
    },
    ...
    init: () => {
      chrome.storage?.local.get(
        ...
        async (result) => {
          ...
          const resolvedSummaryConfig = resolveSummaryConfig(sc.providerId, sc.model);
          ...
          set({
            ...
            summaryProviderId: resolvedSummaryConfig.providerId || '',
            summaryModel: resolvedSummaryConfig.model || '',
            summaryCustomPrompt: customPrompt ?? SUMMARY_SYSTEM_PROMPT,
            enabledMap: newEnabled,
            historyList: allHistory,
          });

          saveSummaryConfig(resolvedSummaryConfig);
        },
      );
