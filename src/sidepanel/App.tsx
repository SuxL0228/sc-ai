...
// 分享功能依赖一台后端服务器。默认留空 = 关闭分享功能。
// 如果你自建了后端，构建时设置环境变量 VITE_API_BASE_URL 指向你的服务器即可自动开启。
const API_BASE_URL = (import.meta.env.VITE_API_BASE_URL || '').replace(/\/+$/, '');
const SHARE_ENABLED = !!API_BASE_URL;
...
const App = () => {
  ...
  const submitNewChannel = (providerId: ProviderId, question: string, s: ReturnType<typeof useStore.getState>) => {
    ...
    const providerHistory = isMultiTurn
    ...
    chrome.runtime?.sendMessage({
      ...
      payload: {
        ...
        settings: {
          ...
          isNewConversation: !isMultiTurn,
          continueChat: isMultiTurn,
        },
      },
    });
  };
  ...
  const messages = useMemo(() => {
    ...
    if (currentQuestion) {
      ...
      if (shouldShowSummary) {
        ...
        const shareButton = SHARE_ENABLED && isSummaryCompleted && hasSummaryContent ? (
          publishedShare ? (
            showButtonText ? (
              <>
                <button
